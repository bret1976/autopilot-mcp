#!/usr/bin/env python3
"""Render the 45–75s Autopilot MCP explainer. Title cards only — no dashboard."""
...
def scene_kicker_title(fonts: dict[str, Path], kicker: str, title: str, sub: str = "") -> Image.Image:
    ...
    # Prefer italic for the second sentence if title has a period split.
    if ". " in title:
...
def scene_workflow(fonts: dict[str, Path]) -> Image.Image:
    ...
    _center(draw, "Not a screen recording. The spine.", serif, 200, INK)
    ...
    _center(draw, "9:16 Shorts  ·  landscape LinkedIn + X", hint, 760, MUTED)
    return im
...
def scene_steps(fonts: dict[str, Path]) -> Image.Image:
    ...
    items = (
        ...
        ("3", "“Run Autopilot.”", "Branded posts go out. No new app."),
    )
    y = 340
...
def scene_why(fonts: dict[str, Path]) -> Image.Image:
    ...
    _center(draw, "Steps locked. Brand theirs.", serif, 300, INK)
    _wrapped(draw, "Language and niche stay in the model they already pay for.", body, 430, MUTED, 1300)
    _wrapped(draw, "No platform to learn. No dashboard to live in.", body, 520, MUTED, 1300)
    return im
...
def scene_cta(fonts: dict[str, Path]) -> Image.Image:
    ...
    _center(draw, "One link. Your model. Your studio.", sans, 680, MUTED)
    return im


def _encode_clip(png: Path, dest: Path, seconds: float) -> None:
    subprocess.check_call(
        [
            ...
            f"{seconds:.2f}",
            "-vf",
            f"fade=t=in:st=0:d=0.35,fade=t=out:st={seconds - 0.35:.2f}:d=0.35,format=yuv420p",
            "-r",
            "30",
            "-c:v",
            "libx264",
...
def render() -> None:
    ...
    scenes = [
        (scene_kicker_title(fonts, "Cold open", "Your buyer already lives in Claude. Stop making them live in your app."), 10),
        (scene_workflow(fonts), 14),
        (scene_steps(fonts), 14),
        (scene_why(fonts), 12),
        (scene_cta(fonts), 12),
    ]
    with tempfile.TemporaryDirectory() as tmp:
        tmp_path = Path(tmp)
        clips: list[Path] = []
        for i, (image, seconds) in enumerate(scenes):
            png = tmp_path / f"scene-{i}.png"
            clip = tmp_path / f"scene-{i}.mp4"
            image.save(png, "PNG")
            if i == 0:
                image.save(PUBLIC / "poster.jpg", "JPEG", quality=88)
            _encode_clip(png, clip, seconds)
            clips.append(clip)
